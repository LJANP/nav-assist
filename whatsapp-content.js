...
function buildSfdcPrompt(touchpoints) {
  ...
  return [
    ...
    '   b. If no phone is present (only a name), do NOT search Contacts by',
    '      name. WhatsApp display names are personal nicknames and do not',
    '      correspond to Salesforce Contact names; a name search returns',
    '      thousands of irrelevant partial matches. Skip the interaction',
    '      immediately and list it under "Needs manual logging".',
    '   c. If a phone is present but zero Contacts match, or more than one',
    '      matches, skip and list under "Unmatched / ambiguous" — do not',
